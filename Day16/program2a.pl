...
sub walk_maze
{
    ...
    my @priority = ({ position => $position, direction => $direction, score => 0, history => [@{$history}] });
    ...
    while (@paths > 0 or @priority > 0)
    {
        ...
        switch ($map->[$y]->[$x])
        {
            ...
            case "."
            {
                ...
                if (is_valid_position($map, { x => $x, y => $y }, $direction))
                {
                    push @paths,
                    {
                        position => { x => $x, y => $y },
...
sub is_valid_position
{
    ...
    my $x = $position->{x} + $direction->{x};
    my $y = $position->{y} + $direction->{y};
