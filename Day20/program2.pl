...
sub get_shortcuts
{
    ...
    for (my $dy = -20; $dy <= 20; $dy++)
    {
        my $new_y = $y + $dy;

        next if $new_y < 0 || $new_y >= scalar(@$map);

        my $max_dx = 20 - abs($dy);
        for (my $dx = -$max_dx; $dx <= $max_dx; $dx++)
        {
            next if $dx == 0 && $dy == 0;

            my $new_x = $x + $dx;
            my $cost = abs($dx) + abs($dy) - 1;

            next if !is_valid_position($map, $new_x, $new_y);

            if (defined $lengths->{$new_x}->{$new_y})
            {
                my $saving = $lengths->{$x}->{$y} - $lengths->{$new_x}->{$new_y} - $cost;
